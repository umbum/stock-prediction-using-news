...
import numpy as np
import pickle
from BERT_GRU_updown import get_model
...
with KK.tf.device('/cpu:0'):
	def load_data(stock_name, time_step):		
		...
		x_label = np.array(x_label) #(2287, 100, 3072)
	
		return (np.array(x_label), np.array(y_temp[100:]),np.array(date[100:]))
	...
	if __name__ == '__main__':
		...
		#print(x_label.shape)#(2287, 100, 3072)
		#print(y_label.shape)
		...
		for epoch in range(parser['epoch']):
			...
			dev_eer = calculate_eer(model, x_dev_label,y_dev_label)
			val_eer = calculate_eer(model, x_val_label,y_val_label)
		

			print('epoch: %d, dev_predict rate: %f, val_predict rate: %f \n'%(int(epoch), dev_eer, val_eer))
			f_eer.write('epoch: %d, dev_val : %f, val_eer: %f \n'%(int(epoch), dev_eer, val_eer))

		
			model.save_weights(save_dir + '%d-%.4f-%.4f-%.4f.h5'%(epoch, hist.history['loss'][0], dev_eer, val_eer))
		f_eer.close()
